"use client";
import React, { useEffect } from 'react';
import AOS from 'aos';
import 'aos/dist/aos.css';

interface ButtonProps {
  name: string;
  icon?: React.ReactNode;  // Ajout de l'option pour l'icône
}

const AboutButton = ({name}: ButtonProps) => {

  useEffect(() => {
    AOS.init({ duration: 1000 }); // Initialisation avec une durée d'animation de 1 seconde
  }, []);

  return (
    <button 
      className="bg-slate-800 no-underline text-gray-200/80 group cursor-pointer relative shadow-2xl shadow-zinc-900 rounded-md p-px text-xs font-semibold leading-6 text-white inline-block"
      data-aos="fade-up"  // L'effet fade-up sera appliqué ici
    >
      <span className="absolute inset-0 overflow-hidden rounded-md">
        <span className="absolute inset-0 rounded-md bg-[image:radial-gradient(75%_100%_at_50%_0%,rgba(56,189,248,0.6)_0%,rgba(56,189,248,0)_75%)] opacity-0 transition-opacity duration-500 group-hover:opacity-100" />
      </span>
      <div className="relative flex space-x-2 items-center z-10 rounded-md bg-zinc-950 py-0.5 px-4 ring-1 ring-white/10">
        <span className='font-rethink text-[15px]'>
          {name}
        </span>
      </div>
      <span className="absolute -bottom-0 left-[1.125rem] h-px w-[calc(100%-2.25rem)] bg-gradient-to-r from-emerald-400/0 via-emerald-400/90 to-emerald-400/0 transition-opacity duration-500 group-hover:opacity-40" />
    </button>
  );
}

export default AboutButton;
